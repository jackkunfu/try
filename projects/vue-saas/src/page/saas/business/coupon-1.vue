                ...
                .form-ctn
                    el-form(:model="editItem" label-width="90px" style="width: 450px")
                        ...
                        el-form-item(label="优惠券序号")
                            el-input(placeholder="请输入优惠券的序号" v-model="editItem.serial_num")

                        .title 步骤二
                        el-form-item(label="优惠券分类")
                            el-select(placeholder="请选择" v-model="editItem.classId"  @click="couponClass()")
                                el-option(v-for="(item,i)  in couponCatelogoList" :label="item.name" :value="item.value" :key="i")
                        
                        el-form-item(label="优惠码标题" v-if="editItem.classId!=0")
                            el-input(type="text" v-model="editItem.codeTitle" placeholder="限25个汉字，显示在B端")

                        el-form-item(label="优惠码数量" v-if="editItem.classId!=0")
                            el-input(type="text" v-model="editItem.codeNum" placeholder="优惠码发放数量")

                        el-form-item(class="search-time" label="码发放时间" v-if="editItem.classId!=0")
                            el-col(:span="11")
                                el-date-picker(type="date" placeholder="选择开始日期" v-model="searchMsg1.startTime" style="width: 100%;")
                            el-col(:span="11")
                                el-date-picker(type="date" placeholder="选择结束日期" v-model="searchMsg1.endTime" style="width: 100%;")
...
export default {
    ...
    data(){
        return {
            ...
            // 编辑的各个字段配置
            editKeys: ['app', 'sortType', 'statu', 'startTime', 'endTime', 'type' ,'classId'],
