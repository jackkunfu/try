<template lang="pug">
    div(style="height:100%;")
        .table-main.no-nav
            ...
            div.edit-ctn(v-if="isInput && !isAdd")
                .page-title {{curOperate}}供应商
                .form-ctn
                    el-form(:model="editItem" label-width="90px" style="width: 450px")
                        el-form-item(label="供应商名称")
                            el-input(type="text" v-model="editItem.vendorName" placeholder="20个以内的汉字和数字组合")

                        el-form-item(label="供应商编码")
                            el-input(type="text" v-model="editItem.vendorCode" placeholder="输入供应商编码")
...
export default {
    ...
    methods: {
        ...
        selfEdit(){
            this.fileList = [{
                name: this.editItem.vendorLogoUrl,
                url: this.editItem.imgUrl
            }];
            this.editItem.logo = this.editItem.vendorLogoUrl;
        },
        ...
        async addAdminAccount(){
            ...
            if(this.newAdminAccount.mobile.trim() == '' || !(/^1[3|4|5|8][0-9]\d{4,8}$/.test(this.newAdminAccount.mobile.trim()))){
                this.$message({
                    type: 'error',
                    message: '手机号有误~'
                })
